...
int UserProcess::execvProcess(const char *path, char *const argv[])
{
  ...
  size_t exec_array_offset_ = array_offset;

  size_t page_for_args = NULL;
  ...
  for(int i = 0; i < argc; i++)
  {
    ...
    write_to_page(page_for_args, next_page_for_args, offset, argv[i], strlen(argv[i]) + 1);

    char* start_next_array_element = (char*)(exec_array_offset_ + i * POINTER_SIZE);
    size_t offset1 = USER_BREAK - 2 * PAGE_SIZE + offset;
  ...
  if(argc > 0)
  {
    ...
    assert(vpn_mapped && "Virtual page for argv was already mapped - in execv");
