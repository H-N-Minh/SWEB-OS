...
int main()
{
    ...
    rv = pj9();         //Try to join thread with itself and try to join already joined thread
    number_of_tests++;
    check_return_value(number_of_tests, rv, &successful_tests, "try to join thread with itself and try to join already joined thread");
