...
// same as shared1, but uses MAP_ANONYMOUS instead of a file descriptor.
// check shared1.c for more details of each test
int shared1_anonym() {
    // //////////////////////////////////////////////// TEST 1 ////////////////////////////////////////////////
    ...
    if (child_pid == 0) {
        ...
        for (int i = 0; i < pages; i++) {
            printf("writing to addr %zu the str (%s)\n", temp, st);
            memcpy((char*) temp, st, strlen(st) + 1);
            temp += 4096;
        }
        printf("child done writing, now reading\n");
        temp = (size_t) addr;
        for (int i = 0; i < pages; i++) {
            printf("reading at addr %zu the str (%s)\n", temp, (char*) temp);
